# -*- coding: utf-8 -*-
"""生成V14(5%/4%)近5年逐年持仓占比及年化收益HTML报告"""
import json

with open('v14_yearly_5y.json', 'r', encoding='utf-8') as f:
    data = json.load(f)

yearly = data['yearly']
assets_order = ['上证50', '创业板50', '纳斯达克100', '沪深300', '中证500', '中证1000', '标普500', '科创50', '国债']
colors = {
    '上证50': '#e53935', '创业板50': '#ff7043', '纳斯达克100': '#00acc1',
    '沪深300': '#8e24aa', '中证500': '#ab47bc', '中证1000': '#3949ab',
    '标普500': '#5c6bc0', '科创50': '#ffa726', '国债': '#7cb342',
}
...
<head>
<meta charset="UTF-8">
<title>V14(5%/4%)近5年逐年持仓占比及收益</title>
...
<body>
<h1>V14 (5%/4%) 策略 — 近5年逐年持仓占比及收益</h1>
<div class="subtitle">标的池: 上证50 + 创业板50 + 纳斯达克100 + 沪深300 + 中证500 + 中证1000 + 标普500 + 科创50 + 国债 | 期间: __START__ ~ __END__</div>
...
<div class="note">
  ℹ️ <b>说明</b><br>
  ① <b>标的池</b>：近5年使用全程可用的8个股票指数（上证50/创业板50/纳斯达克100/沪深300/中证500/中证1000/标普500/科创50）+国债。<br>
  ② <b>年度收益</b> = 当年每日收益累乘 - 1，完整年即年化收益率，不完整年标注天数。<br>
  ③ <b>V14回撤</b> = 当年策略净值距当年内高点的最大回撤（不跨年）。<br>
  ④ <b>持仓占比</b> = 该资产当年持仓天数 / 当年交易日数。国债占比高 = 当年熔断避险时间多。<br>
  ⑤ <b>关键规律</b>：熊市年份（2022）国债占比81%躲过大跌；牛市年份（2024/2025）股票占比提升，吃到主升浪。<br>
  ⑥ <b>注意</b>：2021年只有90天（7月起）、2026年只有125天（截至7月），非完整年。<br>
  ⑦ <b>与近10年对比</b>：近5年多了科创50，V8基线净值走势不同，熔断时点也不同。例如2025年近5年版本V14 +67.29%，而近10年版本V14 +81.26%——标的池差异导致。
</div>

<script>
const YEARLY = __YEARLY__;
const ASSETS = ['上证50','创业板50','纳斯达克100','沪深300','中证500','中证1000','标普500','科创50','国债'];
const COLORS = {"上证50":"#e53935","创业板50":"#ff7043","纳斯达克100":"#00acc1","沪深300":"#8e24aa","中证500":"#ab47bc","中证1000":"#3949ab","标普500":"#5c6bc0","科创50":"#ffa726","国债":"#7cb342"};
...
new Chart(document.getElementById('chart-nav'), {
  ...
  options: {
    ...
    scales: {
      y: { title: { display: true, text: '累计净值' } },
