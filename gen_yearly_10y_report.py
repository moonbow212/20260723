# -*- coding: utf-8 -*-
"""生成V14(5%/4%)近10年逐年持仓占比及年化收益HTML报告"""
import json

with open('v14_yearly_10y.json', 'r', encoding='utf-8') as f:
    data = json.load(f)

yearly = data['yearly']
assets_order = ['上证50', '创业板50', '纳斯达克100', '沪深300', '中证500', '中证1000', '标普500', '国债']
colors = {
    '上证50': '#e53935', '创业板50': '#ff7043', '纳斯达克100': '#00acc1',
    '沪深300': '#8e24aa', '中证500': '#ab47bc', '中证1000': '#3949ab',
    '标普500': '#5c6bc0', '国债': '#7cb342',
}
...
<head>
<meta charset="UTF-8">
<title>V14(5%/4%)近10年逐年持仓占比及收益</title>
...
th, td { padding: 6px 4px; text-align: center; border-bottom: 1px solid #eef0f4; }
...
.note b { color: #1565c0; }
.legend-row { display: flex; gap: 12px; flex-wrap: wrap; margin-bottom: 10px; font-size: 12px; }
.legend-item { display: flex; align-items: center; gap: 5px; }
.legend-swatch { width: 14px; height: 14px; border-radius: 3px; }
</style>
</head>
<body>
<h1>V14 (5%/4%) 策略 — 近10年逐年持仓占比及收益</h1>
<div class="subtitle">标的池: 上证50 + 创业板50 + 纳斯达克100 + 沪深300 + 中证500 + 中证1000 + 标普500 + 国债 | 期间: __START__ ~ __END__</div>
...
<tr>
  ...
  <th rowspan="2">切换次</th>
  <th colspan="8">持仓占比（天数）</th>
</tr>
<tr>
  <th>V14</th><th>V8</th><th>超额</th>
  <th>上证50</th><th>创业板50</th><th>纳指100</th><th>沪深300</th><th>中证500</th><th>中证1000</th><th>标普500</th><th>国债</th>
</tr>
</thead>
...
<div class="note">
  ℹ️ <b>说明</b><br>
  ① <b>标的池</b>：近10年使用全程可用的7个股票指数（上证50/创业板50/纳斯达克100/沪深300/中证500/中证1000/标普500）+国债。<br>
  ② <b>年度收益</b> = 当年每日收益累乘 - 1，完整年即年化收益率，不完整年标注天数。<br>
  ③ <b>V14回撤</b> = 当年策略净值距当年内高点的最大回撤（不跨年）。<br>
  ④ <b>持仓占比</b> = 该资产当年持仓天数 / 当年交易日数。国债占比高 = 当年熔断避险时间多。<br>
  ⑤ <b>关键规律</b>：熊市年份（2018/2022）国债占比极高（83-86%），策略靠避险躲过大跌；牛市年份（2020/2024/2025）股票占比提升，吃到主升浪。<br>
  ⑥ <b>注意</b>：2016年只有89天（7月起）、2026年只有125天（截至7月），非完整年。<br>
  ⑦ <b>与近20年对比</b>：近10年多了创业板50/中证500/标普500三个标的，V8基线净值走势不同，熔断时点也不同。例如2019年近10年版本V14 +24.91%（国债62.7%），而近20年版本V14仅+4.3%（国债100%）。
</div>

<script>
const YEARLY = __YEARLY__;
const ASSETS = ['上证50','创业板50','纳斯达克100','沪深300','中证500','中证1000','标普500','国债'];
const COLORS = {"上证50":"#e53935","创业板50":"#ff7043","纳斯达克100":"#00acc1","沪深300":"#8e24aa","中证500":"#ab47bc","中证1000":"#3949ab","标普500":"#5c6bc0","国债":"#7cb342"};
...
YEARLY.forEach(y => {
  ...
  ASSETS.forEach(a => {
    ...
    if (h && h.pct > 0) {
      ...
      cells += '<td class="pct-cell ' + cls + '">' + h.pct.toFixed(1) + '%<br><span style="font-size:9px;color:#999;">' + h.days + '天</span></td>';
    } else {
...
out = 'V14策略5_4近10年逐年持仓占比及收益.html'
