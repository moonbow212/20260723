# -*- coding: utf-8 -*-
"""
MA20轮动策略回测 V4 —— 买入因子均<0时买入国债指数
策略规则：
  买入因子 = 当日收盘价 / 当日MA20 - 1
  - 两个指数 bf 都 < 0（均跌破MA20）→ 买入国债指数
...
df3 = pd.read_csv('C:/Users/wbl/Desktop/国债.xlsx', sep='\t', encoding='gbk')
...
# ============ 7. 打印结果 ============
print("\n" + "="*100)
print("MA20轮动策略 V4 回测结果（国债指数全程覆盖 + 手续费万分之二/单边）")
print("="*100)
print(f"策略: 买入因子=收盘/MA20-1，持有因子更高者，两者bf均<0时持有国债")
...
for name in ['近10年', '近5年', '近3年', '近1年']:
    ...
    print(f"{'─'*80}")
    print(f"  {'指标':<14} {'轮动策略':>12} {'上证50':>12} {'创业板50':>12} {'国债':>12}")
    ...
    print(f"  持仓: 上证50 {r['hold1_pct']:.1%} | 创业板50 {r['hold2_pct']:.1%} | 国债 {r['hold3_pct']:.1%} | 空仓 {r['cash_pct']:.1%}")
...
with open('C:/Users/wbl/WorkBuddy/2026-07-20-20-22-46/backtest_v4_data.json', 'w', encoding='utf-8') as f:
    json.dump(output, f, ensure_ascii=False)

print("\n数据已导出到 backtest_v4_data.json")
