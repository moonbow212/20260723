# -*- coding: utf-8 -*-
"""
MA20轮动策略回测 V6 —— 三指数轮动(上证50/创业板50/纳斯达克100) + 国债避险
策略规则：
  买入因子 = 当日收盘价 / 当日MA20 - 1
...
df3 = df3[['date', '开盘', '收盘']].rename(columns={'开盘': 'open_3', '收盘': 'close_3'})  # 纳斯达克100
...
# ============ 7. 打印结果 ============
print("\n" + "="*110)
print("MA20轮动策略 V6 回测结果（上证50/创业板50/纳斯达克100 三指数轮动 + 国债避险）")
print("="*110)
...
for name in ['近10年', '近5年', '近3年', '近1年']:
    ...
    print(f"{'─'*90}")
    print(f"  {'指标':<10} {'轮动策略':>12} {'上证50':>11} {'创业板50':>11} {'纳指100':>11} {'国债':>11}")
    ...
    print(f"  持仓: 上证50 {r['hold1_pct']:.1%} | 创业板50 {r['hold2_pct']:.1%} | 纳指100 {r['hold3_pct']:.1%} | 国债 {r['hold4_pct']:.1%} | 空仓 {r['cash_pct']:.1%}")
...
with open('C:/Users/wbl/WorkBuddy/2026-07-20-20-22-46/backtest_v6_data.json', 'w', encoding='utf-8') as f:
    json.dump(output, f, ensure_ascii=False)

print("\n数据已导出到 backtest_v6_data.json")
