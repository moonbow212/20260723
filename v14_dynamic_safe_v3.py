# -*- coding: utf-8 -*-
"""V14动态避险v3策略：黄金价格高于60周均线选黄金ETF，低于选国债（趋势跟随）
...
避险资产选择规则：
  - 黄金ETF收盘价 > 60周均线 → 避险资产 = 黄金ETF
  - 黄金ETF收盘价 <= 60周均线 → 避险资产 = 国债
  - 60周MA不可用时（2014-09之前）默认国债
  
对比3个版本：
  A. 原版：始终国债
  B. 黄金ETF版：2013-07-29起始终黄金ETF
  C. 动态v3：根据黄金vs60周MA趋势跟随选择
"""
import pandas as pd
import numpy as np
import json, os
...
gold_daily['year_week'] = gold_daily['date'].dt.strftime('%Y-W%U')
gold_weekly = gold_daily.groupby('year_week').agg({
    'date': 'last',
    f'close_{GOLD}': 'last'
}).reset_index()
gold_weekly = gold_weekly.sort_values('date').reset_index(drop=True)
gold_weekly['ma60w'] = gold_weekly[f'close_{GOLD}'].rolling(60).mean()
print(f"  周线数据: {gold_weekly['date'].min().date()} ~ {gold_weekly['date'].max().date()}, {len(gold_weekly)}周")
print(f"  60周MA有效起始: {gold_weekly.dropna(subset=['ma60w'])['date'].min().date()}")

# 统计金价在60周MA上方/下方占比
valid = gold_weekly.dropna(subset=['ma60w'])
above_pct = (valid[f'close_{GOLD}'] > valid['ma60w']).mean()
print(f"  金价>60周MA占比: {above_pct*100:.1f}%")

# 将周线60周MA前向填充到日频
gold_ma_daily = gold_weekly[['date', 'ma60w']].dropna(subset=['ma60w']).copy()
gold_ma_daily = gold_ma_daily.rename(columns={'date': 'ma_date'})
print(f"  60周MA日频映射: {len(gold_ma_daily)}条")
...
def build_merged_data(start_date, end_date, safe_mode='bond'):
    ...
    if safe_mode == 'bond':
        ...
        df = pd.merge_asof(
            df.sort_values('date'),
            gold_ma_daily.sort_values('ma_date').rename(columns={'ma_date': 'date'}),
            on='date',
            direction='backward'
        )
        # 趋势跟随：黄金收盘价 > 60周MA → 黄金ETF；<= 60周MA → 国债
        df['safe_haven'] = BOND  # 默认国债
        mask_gold_available = df['date'] >= GOLD_START
        mask_ma_available = df['ma60w'].notna()
        mask_above_ma = df[f'close_{GOLD}'] > df['ma60w']
        # 条件：有黄金数据 + 有60周MA + 收盘价高于MA → 黄金ETF
...
variants = [
    ('bond', '原版(国债避险)'),
    ('gold', '黄金ETF避险(2013起)'),
    ('dynamic', '动态v3(金>60周MA→黄金,金<=60周MA→国债)'),
]
...
with open(os.path.join(BASE_DIR, 'v14_dynamic_safe_v3.json'), 'w', encoding='utf-8') as f:
    json.dump(results, f, ensure_ascii=False, indent=2)
print("\n数据已保存到 v14_dynamic_safe_v3.json")
...
<title>V14动态避险v3：黄金价格vs60周均线趋势跟随选择避险资产</title>
<style>
...
<body>
<h1>V14动态避险v3：黄金价格vs60周均线趋势跟随选择避险资产</h1>
<div class="sub">MA20轮动 · 费率万0.5 · 5%/4%熔断 · 决策bf=(T-1收盘/T-1 MA20)-1 · T日开盘执行 · open-to-open</div>
<div class="note">
<b>避险资产选择规则：</b><br>
&nbsp;&nbsp;• <b>原版</b>：避险资产始终为国债<br>
&nbsp;&nbsp;• <b>黄金ETF版</b>：2013-07-29起避险资产始终为黄金ETF<br>
&nbsp;&nbsp;• <b>动态v3</b>：黄金ETF收盘价 &gt; 60周均线 → 避险选黄金ETF；黄金ETF收盘价 ≤ 60周均线 → 避险选国债<br>
<span style="color:#666">60周MA从2014-09-19起有效，之前默认国债。趋势跟随逻辑：金价处于上升趋势时持有黄金，处于下降趋势时回国债避险。</span>
</div>
'''
...
for pname in ['2013年以来','近20年','近10年','近5年','近3年','近1年']:
    ...
    <thead><tr>
        ...
        <th colspan="3" style="border-right:1px solid #ddd;background:#e8f5e9">动态v3</th>
...
收益口径: open-to-open · 蓝色列=原版(国债) · 黄色列=黄金ETF版 · 绿色列=动态v3<br>
动态v3避险选择：黄金ETF收盘价&gt;60周均线→持黄金ETF，≤60周均线→持国债 | 趋势跟随逻辑 | 60周MA基于周线收盘价计算，前向填充到日频
</div>
</body></html>'''

out_path = os.path.join(BASE_DIR, 'v14_dynamic_safe_v3.html')
