# -*- coding: utf-8 -*-
"""V14动态避险资产策略：黄金价格低于60周均线选黄金ETF，高于选国债
...
def build_merged_data(start_date, end_date, safe_mode='bond'):
    ...
    if safe_mode == 'bond':
        ...
        # 黄金收盘价 < 60周MA → 黄金ETF；>= 60周MA → 国债
        # 2013-07-29之前或60周MA不可用时 → 国债
        df['safe_haven'] = BOND  # 默认国债
        mask_gold_available = df['date'] >= GOLD_START
        mask_ma_available = df['ma60w'].notna()
        mask_below_ma = df[f'close_{GOLD}'] < df['ma60w']
        # 条件：有黄金数据 + 有60周MA + 收盘价低于MA → 黄金ETF
        df.loc[mask_gold_available & mask_ma_available & mask_below_ma, 'safe_haven'] = GOLD
...
variants = [
    ('bond', '原版(国债避险)'),
    ('gold', '黄金ETF避险(2013起)'),
    ('dynamic', '动态避险(金<60周MA→黄金,金>=60周MA→国债)'),
]
...
with open(os.path.join(BASE_DIR, 'v14_dynamic_safe.json'), 'w', encoding='utf-8') as f:
    json.dump(results, f, ensure_ascii=False, indent=2)
print("\n数据已保存到 v14_dynamic_safe.json")
...
<title>V14动态避险策略：黄金vs60周MA选择避险资产</title>
<style>
...
<body>
<h1>V14动态避险策略：黄金价格vs60周均线选择避险资产</h1>
<div class="sub">MA20轮动 · 费率万0.5 · 5%/4%熔断 · 决策bf=(T-1收盘/T-1 MA20)-1 · T日开盘执行 · open-to-open</div>
<div class="note">
<b>避险资产选择规则：</b><br>
&nbsp;&nbsp;• <b>原版</b>：避险资产始终为国债<br>
&nbsp;&nbsp;• <b>黄金ETF版</b>：2013-07-29起避险资产始终为黄金ETF<br>
&nbsp;&nbsp;• <b>动态版</b>：黄金ETF收盘价 &lt; 60周均线 → 避险选黄金ETF；黄金ETF收盘价 ≥ 60周均线 → 避险选国债<br>
<span style="color:#666">60周MA从2014-09-19起有效，之前默认国债。黄金大部分时间在60周MA上方(70.2%)，动态版多数时间使用国债避险。</span>
</div>
'''
...
动态版避险选择：黄金ETF收盘价&lt;60周均线→持黄金ETF，≥60周均线→持国债 | 60周MA基于周线收盘价计算，前向填充到日频
</div>
</body></html>'''

out_path = os.path.join(BASE_DIR, 'v14_dynamic_safe.html')
