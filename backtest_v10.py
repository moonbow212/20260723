# -*- coding: utf-8 -*-
"""
MA20轮动策略回测 V10 —— 八指数轮动 + 国债避险 + 短周期多头排列趋势确认
策略规则：
  买入因子 = 当日收盘价 / 当日MA20 - 1
  趋势确认条件：当日收盘价 > MA10 > MA20（短周期多头排列）
...
def run_backtest(stock_ids, bond_id, start_date, end_date, label):
    ...
    # MA10、MA20和买入因子
    for i in stock_ids:
        df[f'ma10_{i}'] = df[f'close_{i}'].rolling(10).mean()
        ...
        # 趋势确认：close > MA10 > MA20
        df[f'trend_ok_{i}'] = (df[f'close_{i}'] > df[f'ma10_{i}']) & (df[f'ma10_{i}'] > df[f'ma20_{i}'])
...
# ============ 4. 打印结果 ============
print("="*100)
print("MA20轮动策略 V10 回测结果（八指数轮动+国债避险+短周期多头排列close>MA10>MA20）")
print("="*100)
...
with open('C:/Users/wbl/WorkBuddy/2026-07-20-20-22-46/backtest_v10_data.json','w',encoding='utf-8') as f:
    json.dump(output, f, ensure_ascii=False)
print("\n数据已导出到 backtest_v10_data.json")
