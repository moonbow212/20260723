# -*- coding: utf-8 -*-
"""V14(5%/4%)近20年逐年持仓占比及年化收益统计
标的池: 上证50、纳斯达克100、沪深300、中证1000 + 国债 (近20年全程可用)
"""
import pandas as pd
import numpy as np
import json
...
files = {
    1: find_file('上证50'), 3: find_file('纳斯达克100'),
    4: find_file('沪深300'), 6: find_file('中证1000'), 9: find_file('国债'),
}
names = {1:'上证50',3:'纳斯达克100',4:'沪深300',6:'中证1000',9:'国债'}
...
print(f"近20年: {start_date.date()} ~ {last_date.date()}")
...
# 按年统计
all_names = {0:'空仓', 1:'上证50', 3:'纳斯达克100', 4:'沪深300', 6:'中证1000', 9:'国债'}
...
print("\n" + "="*120)
print(f"近20年整体: V14 {total_v14*100:.2f}%, V8 {total_v8*100:.2f}%, 超额 {(total_v14-total_v8)*100:+.2f}%")
print(f"V14年化: {(1+total_v14)**(1/20)-1:.4f}, V8年化: {(1+total_v8)**(1/20)-1:.4f}")
...
output = {
    ...
    'ann_v14': float((1+total_v14)**(1/20)-1),
    'ann_v8': float((1+total_v8)**(1/20)-1),
    'yearly': yearly_data,
...
with open('C:/Users/wbl/WorkBuddy/2026-07-20-20-22-46/v14_yearly.json', 'w', encoding='utf-8') as f:
    json.dump(output, f, ensure_ascii=False, indent=2)
print("\n数据已保存到 v14_yearly.json")
