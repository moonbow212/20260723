# -*- coding: utf-8 -*-
"""
MA20轮动策略回测 V9 —— 八指数轮动 + 国债避险 + 多头排列趋势确认
策略规则：
  买入因子 = 当日收盘价 / 当日MA20 - 1
  趋势确认条件：当日收盘价 > MA20 > MA60（多头排列）
  - 在满足趋势确认的指数中，持有 bf 最高的
  - 若无指数满足趋势确认 → 买入国债指数
...
def run_backtest(stock_ids, bond_id, start_date, end_date, label):
    ...
    for i in stock_ids:
        ...
        df[f'ma60_{i}'] = df[f'close_{i}'].rolling(60).mean()
        df[f'bf_{i}'] = df[f'close_{i}'] / df[f'ma20_{i}'] - 1
        df[f'ratio_{i}'] = df[f'close_{i}'] / df[f'ma20_{i}']
        # 趋势确认：close > MA20 > MA60
        df[f'trend_ok_{i}'] = (df[f'close_{i}'] > df[f'ma20_{i}']) & (df[f'ma20_{i}'] > df[f'ma60_{i}'])
    df = df.dropna(subset=[f'ma60_{i}' for i in stock_ids]).reset_index(drop=True)

    # 信号：在满足趋势确认的指数中选bf最高；都不满足则买国债
    def get_signal(row):
        valid = [i for i in stock_ids if row[f'trend_ok_{i}']]
        if not valid:
            return bond_id
        bfs = {i: row[f'bf_{i}'] for i in valid}
...
# ============ 4. 打印结果 ============
print("="*100)
print("MA20轮动策略 V9 回测结果（八指数轮动+国债避险+多头排列趋势确认）")
print("="*100)
...
for name in ['近10年','近5年','近3年','近1年']:
    ...
    print()
    print(f"  切换{r['switches']}次 | 手续费{r['total_fee']:.2%} | 持仓:", end='')
    for i in all_ids:
...
with open('C:/Users/wbl/WorkBuddy/2026-07-20-20-22-46/backtest_v9_data.json','w',encoding='utf-8') as f:
    json.dump(output, f, ensure_ascii=False)
print("\n数据已导出到 backtest_v9_data.json")
