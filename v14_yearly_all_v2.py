# -*- coding: utf-8 -*-
"""V14(5%/4%)近1/3/5/10/20年逐年统计（动态标的池版）
修正：所有时段均用全部8股+国债，各标的独立计算MA20/bf后left join到国债日历。
有数据时参与选股，无数据时bf=NaN不参与。不再因起始日期不同而排除标的。
"""
import pandas as pd
import numpy as np
import json, os
...
# 导出JSON
with open('C:/Users/wbl/WorkBuddy/2026-07-20-20-22-46/v14_yearly_all_v2.json', 'w', encoding='utf-8') as f:
    json.dump(all_period_results, f, ensure_ascii=False, indent=2)
print("\n数据已保存到 v14_yearly_all_v2.json")
...
<head>
<meta charset="UTF-8">
<title>V14策略(5%/4%) 近1/3/5/10/20年逐年统计（动态标的池）</title>
<style>
...
.note { background:#fff3cd; border:1px solid #ffeaa7; border-radius:6px; padding:10px 16px; margin-bottom:16px; font-size:13px; color:#856404; }
...
<body>
<h1>V14策略 (5%/4%阈值) 逐年统计 — 动态标的池</h1>
<div class="sub">MA20轮动 · 5%回撤触发熔断/4%解除 · T日收盘信号→T+1开盘执行 · 手续费0.02% · 所有时段均用全部8股+国债</div>
<div class="note">⚠️ 修正：各标的在有数据时参与选股（独立计算MA20/bf后left join到国债日历），无数据时bf=NaN不参与。
不再因起始日期不同而排除标的。例如近20年策略中，创业板50从2014年起、标普500从2010年起、科创50从2019年起均可参与选股。</div>
''')
...
动态标的池: 各标的独立计算MA20/bf后left join到国债交易日历，有数据时参与选股，无数据时bf=NaN不参与
</div>
</body></html>''')

html = '\n'.join(html_parts)
out_path = 'C:/Users/wbl/WorkBuddy/2026-07-20-20-22-46/v14_yearly_all_v2.html'
